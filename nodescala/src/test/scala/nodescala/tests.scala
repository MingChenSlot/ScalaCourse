...
class NodeScalaSuite extends FunSuite {
  ...
  test("List[Future[T]] => Future[List[T]]") {
    val all = Future.all(List(Future { 1 }, Future { 2 }, Future { 3 }))

    assert(Await.result(all, 1 second) == List(1, 2, 3))

    val allWithError = Future.all(List(Future { 1 }, Future { 2 }, Future { throw new Exception }))
    try {
      Await.result(allWithError, 1 second)
      assert(false)
    } catch {
      case t: Exception => assert(true)
    }
  }

  test("run with cancellationToken") {
