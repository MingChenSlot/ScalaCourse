...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    /*
    val toWrite = response takeWhile {
      _ => token.nonCancelled
    }
    ...
    */
    ...
    try {
      for (s <- response) {
        exchange.write(s)
        if (token.isCancelled) throw AllDone
      }
    } catch {
      case AllDone =>
    }*/
    ...
    val working = Future.run() { ct=>
      Future {
        while(ct.nonCancelled) {
          async {
            val responsePair = await(listener.nextRequest())
            respond(responsePair._2, ct, handler(responsePair._1))
          }
        }
      }
    }
    Subscription.apply(working, listenerSubscription)
  }
